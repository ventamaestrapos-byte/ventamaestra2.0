...
function checkLicenseOnStartup() {
  // Verificar si existe checkLicense global (de licencias.js)
  if (typeof checkLicense === 'function') {
    const licenseValid = checkLicense();
    if (!licenseValid) {
      alert('⚠️ No se encontró una licencia válida. Será redirigido a la página de activación.');
      window.location.href = 'licencias.html';
      return false;
    }
  } else {
    // Si no está disponible la función, verificar manualmente
    const license = JSON.parse(localStorage.getItem('ventamaestra_license'));
    if (!license || !license.activated) {
      alert('⚠️ Sistema no activado. Será redirigido a la página de licencias.');
      window.location.href = 'licencias.html';
      return false;
    }
    
    // Verificar restricción de dispositivo móvil
    if (license.deviceType === 'Móvil') {
      alert('⚠️ ACCESO DENEGADO\n\nLas ventas no están permitidas en dispositivos móviles.\n\nEste dispositivo solo puede acceder a:\n✅ Productos\n✅ Inventario\n✅ Compras\n✅ Kárdex\n✅ Usuarios\n\n❌ NO PUEDES realizar ventas desde aquí.');
    ...
    if (license.expirationDate) {
      ...
      if (expDate < new Date()) {
        alert('⚠️ La licencia ha expirado. Contacte al administrador.');
        window.location.href = 'licencias.html';
        return false;
      }
    }
    ...
    if (license.storeName) {
      const branchNameEl = document.querySelector('.branch-name');
      if (branchNameEl) {
        branchNameEl.textContent = license.storeName;
      }
    }
    
    if (license.licenseType) {
      const licenseTypeEl = document.querySelector('.license-type');
      if (licenseTypeEl) {
        const typeNames = {
        ...
        licenseTypeEl.textContent = typeNames[license.licenseType] || license.licenseType;
      }
    }
  }
  return true;
